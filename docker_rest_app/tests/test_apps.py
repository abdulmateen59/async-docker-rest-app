...
class TestApp(TestCase):
    ...
    tag: str = '0.0.1'
    docker_url: str = 'http://localhost:8282/api/v2/docker/build'
    task_url: str = 'http://localhost:8282/api/v2/docker/build/task'
    ...
    def test_task_failure(self) -> None:
        ...
        response = requests.get(f'{self.task_url}/{str(task_id)}')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['state'], 'FAILURE')
